...
window.refreshQuestionBankList = () => {
  ...
  filteredBank.forEach((q, i) => {
    ...
    d.innerHTML = `
      ...
      return `
              <div style="display: block; width: 100%; text-align: left; background: ${bg}; color: ${color}; border: 2px solid ${isCorrect ? 'var(--color-success)' : 'var(--color-border)'}; border-radius: var(--radius-md); padding: 1rem 1.5rem; font-size: 1.05rem; font-weight: 500;">
                ${['A', 'B', 'C', 'D', 'E'][idx] || 'X'}) ${opt}
              </div>
            `;
    }).join('')}
